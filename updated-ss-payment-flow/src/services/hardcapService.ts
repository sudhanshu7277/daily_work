...
// FIX (integration issue reported after delivery): the file previously only
// exported the loose `verifyHardCap` function. Some call sites/import
// conventions expect a named service object instead (matching a
// class-instance-like usage: `hardCapService.verifyHardCap(...)`) — this
// named export supports that pattern without changing the underlying
// implementation. Both `verifyHardCap` (function) and `hardCapService`
// (object wrapping it) are now exported; use whichever matches your
// project's import convention.
export const hardCapService = { verifyHardCap };

// Also export as default for projects that prefer `import hardCapService
// from '.../hardCapService'` over a named import.
