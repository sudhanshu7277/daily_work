...
## NEW: Checker/Repair queue entry point (`PaymentWorkflow.tsx`, `PaymentQueueList.tsx`, `queueService.ts`)
Unlike everything else in this package, these three files have **no source
capture behind them at all**. The real Angular app fed checker/repair data
into `payment-parent.component.ts` via `DataSharingService.checkerData` (an
RxJS subscription, confirmed in the captured `ngOnInit`), but the component
that actually *pushes* data into that stream — a queue/list page, possibly
`authorization.component` (seen open in several screenshots alongside
`payment-parent.component.ts`, but never itself photographed) — was never
captured.

`PaymentParent` itself was always ready for this (the `initialCheckerPayload`/
`initialRepairPayload` props existed from the first delivery specifically as
the hook point) — what's new is the *other side*: a reconstructed queue list
UI and service that fetches pending items and feeds them in, so all three
modes are reachable and demonstrable without real backend/routing wired up
yet.

**Before relying on this**: if GAB already has `authorization.component` (or
an equivalent already ported to React) that does this job, use that instead
— just wire its selection handler to call `<PaymentParent
initialCheckerPayload={item}>` / `initialRepairPayload={item.paymentDetailsRequest}`
with the confirmed data shape. `queueService.ts`'s endpoint paths
(`/api/payments/checker/pending`, `/api/payments/repair/pending`) are
**assumed**, following the REST pattern of the other confirmed endpoints —
not verified against anything real. Swap them for your actual endpoints.
