import React, {
  ...
  export const PaymentChild: FC<SSPaymentFlowProps> = ({
    ...
    const setField = useCallback((fieldName: keyof Pain001Model, value: unknown, emitEvent = true) => {
      ...
      if (isRepair && !newlyModifiedFields.includes(fieldName)) {
        setNewlyModifiedFields(prev => [...prev, fieldName]);
      }
    }, [isRepair, newlyModifiedFields, onFormChange]);
  
    // Dual-Blind Key Hydration & Extraction
    useEffect(() => {
    ...
    // Hardcap Limit Integration (400ms Debounce)
    ...
    // Form Validity Evaluation & Output Dispatch
    ...
    const renderField = (
      ...
      const rule = validationResults.get(fieldName);
      if (rule?.visible === false) return null;
      if (paymentInput?.hideFieldsList?.includes(fieldName)) return null;
  
      const value = (formValues as any)[fieldName] ?? '';
      const isRequired = rule?.required ?? configMap.get(fieldName)?.required ?? PAIN001_MANDATORY_FIELDS.includes(fieldName);
      const isReadonly = isFieldReadonly(fieldName);
      const hasDualBlindErr = dualBlindErrors.has(fieldName);
      const isFailed = failedFields.includes(fieldName);
      const isRepairHighlight = isRepair && repairReviewFieldList.includes(fieldName);
      const isNewlyMod = isRepair && newlyModifiedFields.includes(fieldName);
      ...
      return (
        <div
          ...
          onDoubleClick={e => handleDoubleClickFailedField(fieldName, e)}
        >
          <label className={labelClass}>
            {pacsFormVerbiages[fieldName] || defaultLabel}
          ...
          {hasDualBlindErr && (
            <div className="field-error dual-blind-error">{dualBlindErrors.get(fieldName)}</div>
          )}
          {touched[fieldName] && isRequired && !value && (
            <div className="field-error">{opts.errorFallback || `${defaultLabel} is required`}</div>
          )}
          {touched[fieldName] && rule?.pattern && value && !new RegExp(rule.pattern).test(String(value)) && (
