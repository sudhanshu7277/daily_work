...
import { Checker1Component } from './checker1.component';
import { Checker1Service } from 'src/app/shared/services/checker1.service';
import { NumbersOnlyDirective } from 'src/app/shared/directives/numbers-only.directive'; // Fix path if needed
import { ReactiveFormsModule } from '@angular/forms';
import { AgGridModule } from 'ag-grid-angular';
...
describe('Checker1Component', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      // 1. Both the Standalone Component AND its Directive go here
      imports: [
        Checker1Component, 
    ...
    // 2. Mock the grid API to prevent "undefined" errors during render
    component.gridApi = {
      setRowData: jest.fn(),
      sizeColumnsToFit: jest.fn()
    } as any;
  ...
  it('should compile and find the numbers-only directive', () => {
    expect(component).toBeTruthy();
  });
});